  ...
  <div class="container-scroller">
      ...
      <div class="navbar-menu-wrapper d-flex align-items-top"> 
        ...
        <ul class="navbar-nav">
          <li class="nav-item font-weight-semibold d-none d-lg-block ms-0">
            <h1 class="welcome-text"><img src="images/t5.png"> Editar <span class="text-black fw-bold">Encarregado</span></h1>
            <h3 class="welcome-sub-text">Faça a edição das informações do Encarregado</h3>
          </li>
        </ul>
        <!--Fechar header-->
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      if(isset($_POST['editarusuario']))
      {
        ...
        $sql = "UPDATE tb_encarregado SET 
        nome_enc = '$nome', morada_enc = '$morada', telefone_enc = '$telefone', email_enc = '$email' where id_encarregado = $id  ";
        $result = mysqli_query($conexao,$sql);
        if($result)
        {
      ...
      <?php
          $id = $_GET['id'];
          $sql = "SELECT * FROM tb_encarregado WHERE id_encarregado = $id LIMIT 1";
          $result = mysqli_query($conexao,$sql);
          $dados = mysqli_fetch_assoc($result);
      ?>
      <!--Fechar Buscar-->
              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="txtnome" name="txtnome" value="<?php echo $dados['nome_enc'] ?>" Required>
                    </div>
                    <div class="form-group">
                      <strong><label for="Morada">Morada</label></strong>
                      <input type="text" class="form-control" id="txtmorada" name="txtmorada" value="<?php echo $dados['morada_enc'] ?>"  Required>
                    </div>
                    <div class="form-group">
                      <strong><label for="Telefone">Telefone</label></strong>
                      <input type="number" class="form-control" id="txttel" name="txttel" value="<?php echo $dados['telefone_enc'] ?>" Required>
                    </div>
                    <div class="form-group">
                      <strong><label for="Email">Email</label></strong>
                      <input type="email" class="form-control" id="txtemail" name="txtemail" value="<?php echo $dados['email_enc'] ?>" Required>
                    </div>
                    <button type="submit" name="editarusuario" id="cadusuario" class="btn btn-outline-warning btn-fw">Editar</button>
                    <a href="Page_Encarregado.php" class="btn btn-outline-danger btn-fw">Voltar</a>
                  </form>
                </div>
              </div>
            </div>
